...
async def test_team_deactivate_users_reassigns_open_pr_reviewers(client, monkeypatch):
    ...
    r_create = await client.post("/api/v1/pullRequest/create", json={
    ...
    r_deact = await client.post("/api/v1/team/deactivateUsers", json={
        "team_name": "backend",
        "user_ids": ["u2"],
    })
...
async def test_team_deactivate_users_safe_when_no_candidates(client, monkeypatch):
    ...
    r_reassign = await client.post("/api/v1/pullRequest/reassign", json={
        "pull_request_id": "pr_d2",
        "old_reviewer_id": "u2",
    })
...
async def test_team_deactivate_users_team_not_found(client):
    r = await client.post("/api/v1/team/deactivateUsers", json={
        "team_name": "missing",
        "user_ids": ["u1"],
    })
